...
const ProductManagerPage = () => {
  const { data: prData, error, isLoading } = useGetProductQuery();
  const { data: cateData } =  useGetCategoryQuery();
  const newCategory = cateData?.category;
  const newProduct = prData?.product?.docs;
  const [removeProduct] = useRemoveProductMutation();

  const categoryName = (cateId:any) => {
    const cateName = newCategory?.find((item:any)=> item._id === cateId)
    return cateName ? cateName.name : "Not category"
  }
